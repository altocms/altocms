                    ...
                    {if $oComment->getTargetType() != 'talk'}
                        <li id="vote_area_comment_{$oComment->getId()}" class="btn-group vote
                            ...
                            {if {cfg name='view.vote_comments.type'} == 'plus_minus'}
                                <div class="vote-up btn btn-mini" onclick="return ls.vote.vote({$oComment->getId()},this,1,'comment');"><i class="icon-thumbs-up"></i></div>
                                <span class="vote-count btn btn-mini" id="vote_total_comment_{$oComment->getId()}">{if $oComment->getRating() > 0}+{/if}{$oComment->getRating()}</span>
                                <div class="vote-down btn btn-mini" onclick="return ls.vote.vote({$oComment->getId()},this,-1,'comment');"><i class="icon-thumbs-down"></i></div>
                ...
                {if $oUserCurrent}
                    <ul class="comment-actions">
                        {if !$oComment->getDelete() and !$bAllowNewComment}
                            <li class="comment-reply"><a href="#" onclick="ls.comments.toggleCommentForm({$oComment->getId()}); return false;" class="reply-link">{$aLang.comment_answer}</a></li>
                        {/if}

                        {if !$oComment->getDelete() and $oUserCurrent and $oUserCurrent->isAdministrator()}
                            <li class="comment-delete"><a href="#" class="comment-delete red" onclick="ls.comments.toggle(this,{$oComment->getId()}); return false;">{$aLang.comment_delete}</a></li>
                        {/if}

                        {if $oComment->getDelete() and $oUserCurrent and $oUserCurrent->isAdministrator()}
                            <li class="comment-repair"><a href="#" class="comment-repair green" onclick="ls.comments.toggle(this,{$oComment->getId()}); return false;">{$aLang.comment_repair}</a></li>
                        {/if}
