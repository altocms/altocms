...
<form id="photoset-upload-form" method="POST" enctype="multipart/form-data" onsubmit="return false;" class="b-modal_image-upload">
    <header class="b-modal-header">
        <h3>{$aLang.uploadimg}</h3>
        <a href="#" class="b-modal-close jqmClose"></a>
    </header>

    <div id="topic-photo-upload-input" class="b-modal-content topic-photo-upload-input">
...
<form action="" method="POST" enctype="multipart/form-data" id="form-topic-add" class="wrapper-content">
    ...
    {*<ul class="topic-add-tools">
        <li class="topic-add-tools-item_image js-tip-top" title="{$aLang.topic_add_photoset}">
            <a href="#" onclick="ls.topic.toggleAddTools('.topic-create-photoset'); return false;"></a>
        </li>
        <li class="topic-add-tools-item_poll js-tip-top" title="{$aLang.topic_add_poll}">
            <a href="#" onclick="ls.topic.toggleAddTools('.topic-create-poll'); return false;"></a>
        </li>
        <li class="topic-add-tools-item_link js-tip-top" title="{$aLang.topic_add_link}">
            <a href="#" onclick="ls.topic.toggleAddTools('.topic-create-link'); return false;"></a>
        </li>
        {hook run='topic_add_tools_item'}
    </ul>*}
    ...
    <input type="hidden" name="topic_type" value="{$oType->getContentType()}" />

    {hook run='form_add_topic_topic_end'}


    <button type="submit"  name="submit_preview" onclick="ls.topic.preview('form-topic-add','text_preview'); return false;" class="btn  fl-r">{$aLang.topic_create_submit_preview}</button>
    <button type="submit"  name="submit_topic_save" id="submit_topic_save" class="btn">{$aLang.topic_create_submit_save}</button>
    <button type="submit"  name="submit_topic_publish" id="submit_topic_publish" class="btn-primary">{$aLang.topic_create_submit_publish}</button>
</form>
...
{hook run='add_topic_topic_end'}
 ...
 <script>
     $(function(){
         if ($('#swfu_images li').length) {
             ls.topic.toggleAddTools('.topic-create-photoset')
         }
         if ($('[name=question_title]').val()) {
             ls.topic.toggleAddTools('.topic-create-poll')
         }
         if ($('#topic_link_url').val()) {
             ls.topic.toggleAddTools('.topic-create-link')
         }
     });
 </script>
{/block}