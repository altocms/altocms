...
{if count($aTypes)>0}
    <div class="btn-group">
        <a href="{router page='admin'}contentadd/" class="btn btn-primary tip-top" title="{$aLang.action.admin.content_add}"><i class="icon-plus-sign"></i></a>
    </div>
	...
	<tbody class="content">
		{foreach from=$aTypes item=oType}
			<tr id="{$oType->getContentId()}" class="cursor-x">
				<td class="center">
				{$oType->getContentTitle()|escape:'html'}{if !$oType->getContentCandelete()} <em>[{$aLang.action.admin.content_standart}]</em>{/if}
				</td>
				<td class="center">
				...
				<td class="center">
					...
					<a href="{router page='admin'}content/?toggle={if $oType->getContentActive()}off{else}on{/if}&content_id={$oType->getContentId()}&security_ls_key={$ALTO_SECURITY_KEY}">
                        {if $oType->getContentActive()}
                            <i class="icon-ban-circle tip-top" title="{$aLang.action.admin.content_turn_off}"></i>
                        {else}
                            <i class="icon-ok-circle tip-top" title="{$aLang.action.admin.content_turn_on}"></i>
                        {/if}
                    </a>
				</td>
			</tr>
