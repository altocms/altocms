...
F::File_IncludeFile(Config::Get('path.root.engine') . '/lib/external/Sphinx/sphinxapi.php');
...
class PluginSphinx_ModuleSphinx extends Module {
    ...
    protected function InitSphinx() {
        ...
        $this->oSphinx->SetServer(Config::Get('plugin.sphinx.host'), intval(Config::Get('plugin.sphinx.port')));
        /**
    ...
    public function FindContent($sTerms, $sObjType, $iOffset, $iLimit, $aExtraFilters) {
        ...
        $cacheKey = Config::Get('plugin.sphinx.entity_prefix') . "searchResult_{$sObjType}_{$sTerms}_{$iOffset}_{$iLimit}_{$sExtraFilters}";
        if (false === ($data = $this->Cache_Get($cacheKey))) {
             ...
             * Ищем
             */
            if (!is_array($data = $this->oSphinx->Query($sTerms, Config::Get('plugin.sphinx.entity_prefix') . $sObjType . 'Index'))) {
    ...
    public function GetSnippet($sText, $sIndex, $sTerms, $before_match, $after_match) {
        $aReturn = $this->oSphinx->BuildExcerpts(array($sText), Config::Get('plugin.sphinx.entity_prefix') . $sIndex . 'Index', $sTerms, array(
