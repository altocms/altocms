...
class ModuleTopic extends Module {
    ...
    public function GetContentTypeByUrl($sUrl){
		if (false === ($data = $this->Cache_Get("content_type_{$sUrl}"))) {
			...
			$this->Cache_Set($data, "content_type_{$sUrl}", array('content_update','content_new'), 60*60*24*1);
		}
		return 	$data;
    }
	/**
	...
	public function UpdateContentType(ModuleTopic_EntityContent $oType) {
		...
		if ($this->oMapperTopic->UpdateContentType($oType)) {
			...
			//чистим зависимые кеши
			$this->Cache_Clean(Zend_Cache::CLEANING_MODE_MATCHING_TAG,array('content_new','content_update','topic_update'));
	...
	public function AddContentField(ModuleTopic_EntityField $oField) {
		if ($sId=$this->oMapperTopic->AddContentField($oField)) {
			...
			//чистим зависимые кеши
			$this->Cache_Clean(Zend_Cache::CLEANING_MODE_MATCHING_TAG,array('content_new','content_update','field_new','field_update'));
			return $oField;
		}
		return false;
	}
    ...
    public function GetContentFieldById($nId){
		if (false === ($data = $this->Cache_Get("content_field_{$nId}"))) {
			...
			$this->Cache_Set($data, "content_field_{$nId}", array('content_new','content_update','field_new','field_update'), 60*60*24*1);
		}
		return 	$data;
    }
	...
	public function GetTopicsAdditionalData($aTopicId,$aAllowData=null) {
		if (is_null($aAllowData)) {
			$aAllowData=array('user'=>array(),'blog'=>array('owner'=>array(),'relation_user'),'vote','favourite','fields','comment_new');
		}
    ...
    {
        if (!is_array($aTopicsId)) $aTopicsId = array(intval($aTopicsId));
	 ...
	 * @return array
	 */
	public function GetTopicsByType($iPage,$iPerPage,$sType,$bAddAccessible=true,$aTypes=array('personal','open')) {
		$aFilter=array(
			'blog_type' => $aTypes,
			'topic_publish' => 1,
			'topic_type'=>$sType
