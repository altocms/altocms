...
class ModuleSubscribe extends Module {
	...
	public function AddTrackSimple($sTargetType,$sTargetId,$sUserId) {
		...
		if (!($oTrack=$this->Subscribe_GetTrackByTargetAndUser($sTargetType,$sTargetId,$sUserId))) {
			...
			$oTrack->setKey(func_generator(32));
			$oTrack->setIp(func_getIp());
			$oTrack->setStatus(1);
	...
	public function GetTracks($aFilter,$aOrder,$iCurrPage,$iPerPage) {
		return array('collection'=>$this->oMapper->GetTracks($aFilter,$aOrder,$iCount,$iCurrPage,$iPerPage),'count'=>$iCount);
	}
	/**
	...
	public function GetTrackByTargetAndUser($sTargetType,$iTargetId,$sUserId) {
		$aRes=$this->GetTracks(array('target_type'=>$sTargetType,'target_id'=>$iTargetId,'user_id'=>$sUserId),array(),1,1);
		if (isset($aRes['collection'][0])) {
			return $aRes['collection'][0];
		}
		return null;
