...
class ModulePage_MapperPage extends Mapper {
	...
	public function GetNextPageBySort($iSort,$sPid,$sWay) {
		...
		$sql = "SELECT * FROM ".Config::Get('db.table.page')." WHERE { page_pid = ? and } {$sPidNULL} page_sort {$sWay} ? order by page_sort {$sOrder} limit 0,1";
	...
	public function GetMaxSortByPid($sPid) {
		...
		$sql = "SELECT max(page_sort) as max_sort FROM ".Config::Get('db.table.page')." WHERE 1=1 { and page_pid = ? } {$sPidNULL} ";
