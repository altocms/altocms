...
//		$this->AddEventPreg('/^top$/i','/^(page([1-9]\d{0,5}))?$/i',array('EventTopics','topics'));
	...
	protected function EventShowTopic() {
        ...
        if (!$oTopic->getPublish() && !Config::Get('module.topic.draft_link')) {
            if (!$this->oUserCurrent or ($this->oUserCurrent->getId()!=$oTopic->getUserId() and !$this->oUserCurrent->isAdministrator())) {
                return parent::EventNotFound();
            }
        }
		/**
    ...
    protected function AjaxUpdateComment()
    {
        ...
        // * Проверка на дублирующий коммент
        if ($this->Comment_GetCommentUnique($oComment->getTargetId(), $oComment->getTargetType(), $this->oUserCurrent->getId(), $oComment->getParentId(), md5($sNewText))) {
            $this->Message_AddErrorSingle($this->Lang_Get('topic_comment_spam'), $this->Lang_Get('error'));
            return;
        }
