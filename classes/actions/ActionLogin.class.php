        ...
        if ((F::CheckVal($sUserLogin, 'mail') && $oUser = $this->User_GetUserByMail($sUserLogin))  ||  ($oUser = $this->User_GetUserByLogin($sUserLogin))) {
            ...
            if ($oUser->IsBanned()) {
                if ($oUser->IsBannedByIp()) {
                    ...
                    $this->Message_AddErrorSingle($this->Lang_Get('user_banned_before', array('date' => $oUser->GetBanLine())));
                    return;
                } else {
            ...
            if ($this->Security_CheckSalted($oUser->getPassword(), $sUserPassword, 'pass')) {
                if (!$oUser->getActivate()) {
                    $this->Message_AddErrorSingle(
                        $this->Lang_Get('user_not_activated', array('reactivation_path' => Router::GetPath('login') . 'reactivation'))
                    );
                    return;
                }
	...
	protected function EventReminder() {
		...
		if (func_check($this->GetParam(0),'md5')) {
			...
			if ($oReminder=$this->User_GetReminderByCode($this->GetParam(0))) {
				if (!$oReminder->getIsUsed() && strtotime($oReminder->getDateExpire())>time() && $oUser=$this->User_GetUserById($oReminder->getUserId())) {
