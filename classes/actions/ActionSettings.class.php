	...
	protected function EventAccount() {
		...
		if (isPost('submit_account_edit')) {
            ...
            if ($sPassword = $this->GetPost('password')) {
                if (($nMinLen = Config::Get('module.security.password_len')) < 3) $nMinLen = 3;
                if (F::CheckVal($sPassword, 'password', $nMinLen)) {
                    if ($sPassword == $this->GetPost('password_confirm')) {
                        if ($this->Security_CheckSalted($this->oUserCurrent->getPassword(), $this->GetPost('password_now'), 'pass')) {
                    ...
                    $bError = true;
                    $this->Message_AddError($this->Lang_Get('settings_profile_password_new_error', array('num' => $nMinLen)), $this->Lang_Get('error'));
                }
            }
            /**
