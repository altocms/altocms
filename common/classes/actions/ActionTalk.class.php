...
class ActionTalk extends Action {
    ...
    protected function EventAdd() {
        ...
        if ($oTalk = $this->Talk_SendTalk(
            ...
            $this->CheckTalkImages($oTalk, $oTalk->getId());
     ...
     * @return bool
     */
    public function CheckTalkImages($oComment, $nId) {
        ...
        if (preg_match_all("~0u\w{8}-\w{8}-\w{8}~", $oComment->getText(), $aUuid) && isset($aUuid[0])) {
            ...
            if ($aNewResources) {
                $this->Mresource_AddTargetRel($aNewResources, 'talk', $nId);
            }
            ...
            $aMresources = $this->Mresource_GetMresourcesRelByTarget('talk', $nId);
    ...
    protected function EventRead() {
        ...
        $this->Viewer_Assign('bAllowToComment', true);
    ...
    protected function checkTalkFields() {
        ...
        foreach ($aUsers as $sUser) {
            ...
            if (($oUser = $this->User_GetUserByLogin($sUser)) && $oUser->getActivate() == 1) {
                ...
                $this->Message_AddError(
                    $this->Lang_Get('talk_create_users_error_not_found') . ' «' . htmlspecialchars($sUser) . '»',
                    $this->Lang_Get('error')
                );
                $bOk = false;
            }
