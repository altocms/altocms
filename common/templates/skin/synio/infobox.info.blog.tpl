...
<ul class="blog-info">
	...
	<li><span>{if $oBlog->getType()!='personal'}<a href="{$oBlog->getUrlFull()}users/">{$aLang.infobox_blog_users}</a>{else}{$aLang.infobox_blog_users}{/if}</span> <strong>{$oBlog->getCountUser()}</strong></li>
