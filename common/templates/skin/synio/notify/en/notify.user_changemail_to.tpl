You have sent a request to change user email <a href="{$oUser->getUserWebPath()}">{$oUser->getLogin()}</a> at <a href="{Config::Get('path.root.url')}">{Config::Get('view.name')}</a>.<br/>
