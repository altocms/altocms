<header id="header" role="banner">
	...
	<ul class="nav nav-main" id="nav-main">
		<li {if $sMenuHeadItemSelect=='blog'}class="active"{/if}><a href="{cfg name='path.root.web'}">{$aLang.blog_menu_all}</a> <i></i></li>
		{if count($aContentTypes)>1}
			{foreach from=$aContentTypes item=oContentType}
				<li {if $sMenuHeadItemSelect=='filter' && $sEvent==$oContentType->getContentUrl()}class="active"{/if}><a href="{router page='filter'}{$oContentType->getContentUrl()}/">{$oContentType->getContentTitleDecl()|escape:'html'}</a> <i></i></li>
			{/foreach}
		{/if}
