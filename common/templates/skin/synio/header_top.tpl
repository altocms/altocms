<header id="header" role="banner">
    ...
    <ul class="nav nav-main" id="nav-main">
        ...
        {if count($aContentTypes)>1}
            {foreach from=$aContentTypes item=oContentType}
                <li {if $sMenuHeadItemSelect=='filter' AND $sEvent==$oContentType->getContentUrl()}class="active"{/if}><a
                            href="{router page='filter'}{$oContentType->getContentUrl()}/">{$oContentType->getContentTitleDecl()|escape:'html'}</a>
                    <i></i></li>
            {/foreach}
        {/if}
        ...
        <li {if $sMenuHeadItemSelect=='stream'}class="active"{/if}><a
                    href="{router page='stream'}">{$aLang.stream_menu}</a> <i></i></li>
    ...
    {if $oUserCurrent}
        <div class="dropdown-user" id="dropdown-user">
            ...
            <ul class="dropdown-user-menu" id="dropdown-user-menu" style="display: none">
                <li class="item-stat">
                    <span class="strength" title="{$aLang.user_skill}"><i
                                class="icon-synio-star-green"></i> {$oUserCurrent->getSkill()}</span>
                    <span class="rating {if $oUserCurrent->getRating() < 0}negative{/if}"
                          title="{$aLang.user_rating}"><i
                ...
                <li class="item-messages">
                    <a href="{router page='talk'}" id="new_messages">
                        ...
                        {if $iUserCurrentCountTalkNew}
                            <div class="new">+{$iUserCurrentCountTalkNew}</div>{/if}
                    </a>
                </li>
                ...
                <li class="item-create"><i class="item-icon"></i><a
                            href="{router page='content'}topic/add/">{$aLang.block_create}</a></li>
                {hook run='userbar_item_last'}
                <li class="item-signout"><i class="item-icon"></i><a
    ...
    {if $iUserCurrentCountTalkNew}
    <a href="{router page='talk'}" class="new-messages">+{$iUserCurrentCountTalkNew}
        <iclass="icon-synio-new-message"></i>
    </a>
    {/if}
    {if $iUserCurrentCountTrack}
    <a href="{router page='feed'}track/new/" class="new-track">+{$iUserCurrentCountTrack}
        <i class="icon-synio-comments-green"></i>
    </a>
    {/if}
