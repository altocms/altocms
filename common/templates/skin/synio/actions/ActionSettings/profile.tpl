...
<form method="post" enctype="multipart/form-data" class="form-profile">
    <div class="wrapper-content">
        <p id="profile_user_field_template" style="display:none;" class="js-user-field-item">
            ...
            <a class="icon-synio-remove" title="{$aLang.user_field_delete}" href="#"
        ...
        <div class="avatar-change">
            ...
            <div>
                <a href="#" id="avatar-upload"
                   class="link-dotted">{if $oUserCurrent->getProfileAvatar()}{$aLang.settings_profile_avatar_change}{else}{$aLang.settings_profile_avatar_upload}{/if}</a><br/>
                <a href="#" id="avatar-remove" class="link-dotted" onclick="return ls.user.removeAvatar();"
                   style="{if !$oUserCurrent->getProfileAvatar()}display:none;{/if}">{$aLang.settings_profile_avatar_delete}</a>
            </div>
        ...
        <dl class="form-item">
            ...
            <dd>
                <input type="text" name="profile_name" id="profile_name"
                       value="{$oUserCurrent->getProfileName()|escape:'html'}" class="input-text input-width-250">
            </dd>
        </dl>
        ...
        <dl class="form-item">
            ...
            <dd>
                <select name="profile_sex" id="profile_sex" class="input-width-250">
                    <option value="man"
                            {if $oUserCurrent->getProfileSex()=='man'}selected{/if}>{$aLang.settings_profile_sex_man}</option>
                    <option value="woman"
                            {if $oUserCurrent->getProfileSex()=='woman'}selected{/if}>{$aLang.settings_profile_sex_woman}</option>
                    <option value="other"
                            {if $oUserCurrent->getProfileSex()=='other'}selected{/if}>{$aLang.settings_profile_sex_other}</option>
                </select>
            </dd>
        </dl>
                ...
                <select name="profile_birthday_day" style="width: 58px; margin-right: 4px;">
                    ...
                    {section name=date_day start=1 loop=32 step=1}
                        <option value="{$smarty.section.date_day.index}"
                                {if $smarty.section.date_day.index==$oUserCurrent->getProfileBirthday()|date_format:"%d"}selected{/if}>{$smarty.section.date_day.index}</option>
                    {/section}
                </select>
                ...
                <select name="profile_birthday_month" style="width: 103px; margin-right: 4px;">
                    ...
                    {section name=date_month start=1 loop=13 step=1}
                        <option value="{$smarty.section.date_month.index}"
                                {if $smarty.section.date_month.index==$oUserCurrent->getProfileBirthday()|date_format:"%m"}selected{/if}>{$aLang.month_array[$smarty.section.date_month.index][0]}</option>
                    {/section}
                </select>
                ...
                <select name="profile_birthday_year" style="width: 72px">
                    ...
                    {section name=date_year loop=$smarty.now|date_format:"%Y"+1 max=$smarty.now|date_format:"%Y"-2012+130 step=-1}
                        <option value="{$smarty.section.date_year.index}"
                                {if $smarty.section.date_year.index==$oUserCurrent->getProfileBirthday()|date_format:"%Y"}selected{/if}>{$smarty.section.date_year.index}</option>
                    {/section}
                </select>
            </dd>
        </dl>
        ...
        <textarea name="profile_about" id="profile_about" class="input-text input-width-full"
                  rows="7">{$oUserCurrent->getProfileAbout()|escape:'html'}</textarea>
        <br/>
        <br/>
        <br/>
        ...
        {if count($aUserFieldValues)}
            {foreach $aUserFieldValues as $oField}
                <dl class="form-item">
                    <dt><label for="profile_user_field_{$oField->getId()}">{$oField->getTitle()|escape:'html'}:</label>
                    </dt>
                    <dd><input type="text" class="input-text input-width-300"
                               name="profile_user_field_{$oField->getId()}" id="profile_user_field_{$oField->getId()}"
                               value="{$oField->getValue()|escape:'html'}"/></dd>
                </dl>
            {/foreach}
        {/if}
    </div>
        ...
        <div id="user-field-contact-contener">
            {foreach $aUserFieldContactValues as $oField}
                <p class="js-user-field-item">
                    ...
                    <input type="text" name="profile_user_field_value[]" value="{$oField->getValue()|escape:'html'}"
    ...
    <div class="wrapper-content">
        ...
        <button type="submit" name="submit_profile_edit" class="button button-primary"/>
