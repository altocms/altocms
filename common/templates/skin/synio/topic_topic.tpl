...
{if $oContentType->isAllow('photoset')}
    ...
    {if $oMainPhoto}
        <div class="topic-photo-preview" id="photoset-main-preview-{$oTopic->getId()}"
             onclick="window.location='{$oTopic->getUrl()}#photoset'">
            <div class="topic-photo-count" id="photoset-photo-count-{$oTopic->getId()}">
                {$oTopic->getPhotosetCount()} {$aLang.topic_photoset_photos}
            </div>

            {if $oMainPhoto->getDescription()}
                <div class="topic-photo-desc" id="photoset-photo-desc-{$oTopic->getId()}">
...
{if $oContentType->isAllow('photoset') AND !$bTopicList AND $iPhotosCount}
    <script type="text/javascript"
            src="{cfg name='path.frontend.url'}/libs/js/vendor/prettyphoto/js/jquery.prettyphoto.js"></script>
    <link rel='stylesheet' type='text/css'
          href="{cfg name='path.frontend.url'}/libs/js/vendor/prettyphoto/css/prettyphoto.css"/>
    <script type="text/javascript">
    ...
    <div class="topic-photo-images">
        ...
        <ul id="topic-photo-images">
            {assign var=aPhotos value=$oTopic->getPhotosetPhotos(0, Config::Get('module.topic.photoset.per_page'))}
            {if count($aPhotos)}
                {foreach from=$aPhotos item=oPhoto}
                    <li><a class="photoset-image" href="{$oPhoto->getWebPath(1000)}" rel="[photoset]"
                           title="{$oPhoto->getDescription()}"><img src="{$oPhoto->getWebPath('50crop')}"
                                                                    alt="{$oPhoto->getDescription()}"/></a></li>
        ...
        {if count($aPhotos)<$oTopic->getPhotosetCount()}
            <a href="javascript:ls.photoset.getMore({$oTopic->getId()})" id="topic-photo-more"
               class="topic-photo-more">{$aLang.topic_photoset_show_more} &darr;</a>
        {/if}
    </div>
{/if}

{if $oContentType->isAllow('question') AND $oTopic->getQuestionAnswers()}
    <div class="poll-zone">
            ...
            {if !$oTopic->getUserQuestionIsVote()}
                <ul class="poll-vote">
                    {foreach $oTopic->getQuestionAnswers() AS $key=>$aAnswer}
                        <li><label><input type="radio" id="topic_answer_{$oTopic->getId()}_{$key}"
...
{if $oContentType->isAllow('link') AND $oTopic->getLinkUrl()}
    <div class="topic-url">
        {$aLang.topic_link_create_url}: <a href="{router page='content'}go/{$oTopic->getId()}/"
