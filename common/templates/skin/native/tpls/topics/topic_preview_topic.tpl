...
<article class="topic topic-type-{$oTopic->getType()}">
	...
	<footer class="topic-footer">
		...
		<ul class="topic-info">
			<li class="topic-info-author"><a rel="author" href="{$oUser->getProfileUrl()}">{$oUser->getLogin()}</a></li>
...
<button type="submit" name="submit_topic_publish" class="btn-primary fl-r" onclick="jQuery('#submit_topic_publish').trigger('click');">
...
<button type="button" name="submit_preview" class="btn js-topic-preview-text-hide-button">{$aLang.topic_create_submit_preview_close}</button>
