	...
	<form action="" method="POST" enctype="multipart/form-data" id="form-topic-add" class="js-form-validate">
		...
		<p><label for="topic_url">{$aLang.topic_create_url}:</label>
			<span class="b-topic-url-demo">{$_aRequest.topic_url_before}</span><span
					class="b-topic_url_demo-edit">{$_aRequest.topic_url}</span>{if $_aRequest.topic_url  AND E::IsAdmin()}<input
			...
			<button class="btn js-tip-help" title="{$aLang.topic_create_url_edit}" onclick="ls.topic.editUrl(this); return false;"><i class="icon-edit"></i></button>
			{/if}
			<button class="btn js-tip-help" title="{$aLang.topic_create_url_short}" onclick="ls.topic.shortUrl('{$_aRequest.topic_url_short}'); return false;"><i class="icon-share-alt"></i></button>
		...
		{if $oType->isAllow('photoset')}
			...
			<h2 class="page-header"><a class="link-dotted pointer" onclick="$('.photoset-upload').slideToggle();return false;">{$aLang.topic_toggle_images}</a></h2>
			...
			<div class="fieldset photoset-upload" {if !count($aPhotos)}style="display:none;"{/if}>
		...
		{if $oType->isAllow('question')}
			<h2 class="page-header"><a class="link-dotted pointer" onclick="$('.js-poll-add').slideToggle();return false;">{$aLang.topic_toggle_poll}</a></h2>

			<div class="poll-add js-poll-add" {if !$_aRequest.question_title}style="display:none;"{/if}>
		...
		{if $oType->isAllow('link')}
			...
			<div class="poll-add" id="topic-link" {if !$_aRequest.topic_link_url}style="display:none;"{/if}>
