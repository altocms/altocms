...
{block name='layout_content'}
	...
	<form action="" method="POST" enctype="multipart/form-data">
		...
		<button type="submit" class="btn-primary" name="submit_talk_add">{$aLang.talk_create_submit}</button>
		<button type="submit" class="btn" name="submit_preview" onclick="jQuery('#text_preview').show(); ls.tools.textPreview('talk_text',false); return false;">{$aLang.topic_create_submit_preview}</button>
	</form>
