        ...
        <header id="header" role="banner">
            ...
            <nav id="userbar" class="clearfix">
                {if $iUserCurrentCountTalkNew}
                    <div class="messages">
                        {$aLang.userbar_messages_you_have} {$iUserCurrentCountTalkNew} {$iUserCurrentCountTalkNew|declension:$aLang.userbar_messages_new_new_count_declension:'russian'}
                        <a href="{router page='talk'}" class="new-messages" id="new_messages"
                           title="{if $iUserCurrentCountTalkNew}{$aLang.user_privat_messages_new}{/if}">{$iUserCurrentCountTalkNew|declension:$aLang.userbar_messages_new_count_declension:'russian'}</a>
                    </div>
                {/if}
                ...
                <div class="menu">
                    {if $oUserCurrent}
                        {$aLang.userbar_hello},
                        <a href="{$oUserCurrent->getProfileUrl()}" class="username">{$oUserCurrent->getLogin()}</a>
                        <div class="usermenu-trigger"><i></i></div>
                        <div class="dropdown-usermenu">
                            <a href="{$oUserCurrent->getProfileUrl()}"><img src="{$oUserCurrent->getAvatarUrl(24)}"
                                                                            alt="avatar" class="avatar"/></a>
                            <a href="{$oUserCurrent->getProfileUrl()}" class="user-title">{$aLang.user_menu_profile}</a>

                            <ul class="links">
                                <li></li>
                                <li><i class="icon-native-usermenu-favourites"></i><a
                                            href="{$oUserCurrent->getProfileUrl()}favourites/topics/">{$aLang.user_menu_profile_favourites}</a>
                                </li>
                                <li><i class="icon-native-usermenu-talk"></i><a
                                            href="{router page='talk'}">{$aLang.talk_menu_inbox}</a></li>
                                <li><i class="icon-native-usermenu-settings"></i><a
                                            href="{router page='settings'}">{$aLang.settings_menu}</a></li>
                                {hook run='userbar_item'}
                            </ul>

                            <ul class="links noborder nopad">
                                <li><i class="icon-native-usermenu-logout"></i><a
                                            href="{router page='login'}exit/?security_ls_key={$ALTO_SECURITY_KEY}">{$aLang.exit}</a>
                                </li>
                            </ul>
                        </div>
                    {else}
                        ...
                        <a href="#" data-toggle="modal" data-target="#modal-login"
                           onclick="jQuery('[href=#tab-pane-login]').tab('show');">{$aLang.user_login_submit}</a>
                        или
                        <a href="#" data-toggle="modal" data-target="#modal-login"
                           onclick="jQuery('[href=#tab-pane-registration]').tab('show');">{$aLang.registration_submit}</a>
                    {/if}
                </div>

                <form action="{router page='search'}topics/" class="search-form">
                    <input type="text" placeholder="{$aLang.search}" maxlength="255" name="q"
                           class="search-form-input width-full">
                </form>
            </nav>
