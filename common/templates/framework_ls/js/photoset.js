...
ls.photoset =( function ($) {
	...
	{
		ls.ajax(aRouter['content']+'setimagedescription', {'id':id, 'text':text},  function(result){
	...
	this.getMore = function(topic_id)
	{
		...
		ls.ajax(aRouter['content']+'getmore', {'topic_id':topic_id, 'last_id':this.idLast}, function(result){
			this.isLoading=false;
			if (!result.bStateError) {
				if (result.photos) {
	...
	{
		ls.photoset.addPhotoEmpty();
		ls.ajaxSubmit(aRouter['content']+'upload/',$('#photoset-upload-form'),function(data){
			if (data.bStateError) {
				$('#photoset_photo_empty').remove();
