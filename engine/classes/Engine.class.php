...
F::IncludeFile(Config::Get('path.root.engine') . '/lib/internal/ProfilerSimple/Profiler.class.php');
    ...
    public static function GetClassPath($oObject) {
        ...
        if ($aInfo[self::CI_ENTITY]) {
            ...
            if ($aInfo[self::CI_PLUGIN]) {
                ...
                $sPath .= 'plugins/' . F::StrUnderscore($aInfo[self::CI_PLUGIN])
            ...
            if ($aInfo[self::CI_PLUGIN]) {
                ...
                // Блок ядра
                $sPath .= 'classes/widgets/Widget' . $aInfo[self::CI_WIDGET] . '.class.php';
            }
        ...
        return F::File_Exists($sPath) ? F::File_NormPath($sPath) : null;
    }
