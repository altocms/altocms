...
F::IncludeFile(Config::Get('path.root.engine').'/lib/external/DklabCache/config.php');
...
class ModuleCache extends Module {
	...
	public function Init() {
		...
		if ($this->sCacheType==SYS_CACHE_TYPE_FILE) {
            F::File_CheckDir(Config::Get('sys.cache.dir'), true);
     ...
     * @return  bool
     */
    public function Set($data, $sName, $aTags = array(), $nTimeLife = false)
    {
        // Если кеш не используется или модуль завершил свою работу, то ничего не кешируется
        if (!$this->bUseCache OR $this->isDone()) {
            return false;
        }
        ...
        return $this->oBackendCache->save($data, $sName, $aTags, $nTimeLife);
    }
    ...
    public function SmartSet($data, $sName, $aTags = array(), $nTimeLife = false)
    {
        ...
        $this->Set($data, $this->sPrefixSmartCache . $sName, array(), $nTimeLife !== false ? $nTimeLife + 60 : false);
    ...
    public function Clean($sMode = Zend_Cache::CLEANING_MODE_ALL, $aTags = array())
    {
        ...
        return $this->oBackendCache->clean($sMode, $aTags);
    }
    ...
    {
        if (!is_array($aTags)) $aTags = array((string)$aTags);
